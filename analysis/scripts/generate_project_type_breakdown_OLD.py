...
def generate_project_type_pdf_report(df_10yr, df_5yr, award_type):
    ...
    try:
        # Check common paths or project paths
        font_paths = [
            '/Users/shivpat/seed-fund-tracking/fonts/Montserrat-Regular.ttf',
            '/Users/shivpat/seed-fund-tracking/assets/fonts/Montserrat-Regular.ttf',
        ...
        if found_font:
            pdfmetrics.registerFont(TTFont('Montserrat', found_font))
            # Try to register bold version too
            bold_path = found_font.replace('Regular', 'Bold')
            if os.path.exists(bold_path):
                pdfmetrics.registerFont(TTFont('Montserrat-Bold', bold_path))
                font_name = 'Montserrat'
            else:
                font_name = 'Montserrat'
                print("Warning: Montserrat-Bold not found, using Regular for bold")
        else:
            print("Warning: Montserrat font file not found. Using Helvetica.")
    ...
    title_style = ParagraphStyle(
        ...
        fontName=font_name + '-Bold' if font_name == 'Montserrat' else 'Helvetica-Bold',
        fontSize=18,
        textColor=rl_colors.HexColor(IWRC_COLORS['primary']),
        spaceAfter=30,
        alignment=1  # Center
    )
    ...
    normal_style = ParagraphStyle(
        ...
        fontName=font_name,
        fontSize=10,
        textColor=rl_colors.HexColor(IWRC_COLORS['text'])
    )
    ...
    metrics_table.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (-1, 0), rl_colors.HexColor(IWRC_COLORS['primary'])),
        ('TEXTCOLOR', (0, 0), (-1, 0), rl_colors.white),
        ('ALIGN', (0, 0), (-1, -1), 'LEFT'),
        ('FONTNAME', (0, 0), (-1, 0), font_name + '-Bold' if font_name == 'Montserrat' else 'Helvetica-Bold'),
        ('FONTSIZE', (0, 0), (-1, 0), 11),
        ('BOTTOMPADDING', (0, 0), (-1, 0), 12),
        ('BACKGROUND', (0, 1), (-1, -1), rl_colors.HexColor(IWRC_COLORS['background'])),
        ('GRID', (0, 0), (-1, -1), 1, rl_colors.HexColor(IWRC_COLORS['neutral_light'])),
        ('FONTNAME', (0, 1), (-1, -1), font_name),
        ('FONTSIZE', (0, 1), (-1, -1), 9),
        ('TEXTCOLOR', (0, 1), (-1, -1), rl_colors.HexColor(IWRC_COLORS['text'])),
    ]))
